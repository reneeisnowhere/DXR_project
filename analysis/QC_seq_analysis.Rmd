...
# get_peak_widths <- function(histone_name, base_path) {
#   path <- file.path(base_path, histone_name, "macs2_output")
#   
#   file_list <- list.files(path = path, pattern = "\\.narrowPeak$", full.names = TRUE)
#   
#   peak_list <- map(file_list, ~ {
#     fread(.x, header = FALSE) %>%
#       transmute(width = abs(V3 - V2))
#   })
# 
#   names(peak_list) <- paste0(histone_name, "_", tools::file_path_sans_ext(basename(file_list)))
# 
#   peak_widths <- bind_cols(peak_list)
# 
#   return(peak_widths)
# }
# all_widths_list <- map(histone_list, ~ get_peak_widths(.x, base_path))
#   
```
...
get_peak_widths_long <- function(histone_name, base_path) {
  path <- file.path(base_path, histone_name, "macs2_output")
  
  file_list <- list.files(path = path, pattern = "\\.(narrowPeak|broadPeak)$", full.names = TRUE)
   # Exclude files with 'all_peaks' in the name
  file_list <- file_list[!grepl("all_peaks", basename(file_list))]

  
  peak_list <- purrr::map(file_list, function(file_path) {
...
all_peaks_long <- map_dfr(histone_list, ~ get_peak_widths_long(.x, base_path))

anno_peak_width_long <- all_peaks_long %>% 
  mutate(file= gsub("_peaks.narrowPeak","",file)) %>% 
